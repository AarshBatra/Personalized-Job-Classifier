...
Later versions of this app would be less buggy and most probably involve a user login (as it is a 'Personalized' Job Classifier). Until then, experience this version:-), but first read the paper and the coded simulation. [Click
[here](https://aarsh.shinyapps.io/personalized-job-classifier/) to view the app].
...
```{r packagesInfo, message = FALSE, warning = FALSE}
...
Next, lets take a look at the code outline. (Go on to the next page)
...
```{r figure1CodeOutline, out.width = "80%", fig.align = "center", echo = FALSE}
# Path as per hp laptop (change if necessary when using sony laptop)
knitr::include_graphics("C:/Users/Aarsh/Dropbox/RA Docs/2018-19/PJCFilesTamperingWithAppDeploymentProcess")
```

&nbsp;

**Part-1** (Steps:2-14, Figure 1) builds 100 models (M~1~ to M~100~) corresponding to 100 companies (C~1~ to C~100~). Also, in every iteration (one pass through the for loop shown in Figure 1) it accumalates the personalized labels correponding to User1 in `myPersonalizedLabels` vector (Remember the personalized label for User1 correponding to C~n~ is obtained by passing the **Applicant Feature Vector** of User1 through M~n~). 
...
```{r paramInit, message = FALSE, warning = FALSE}
# used later to calculate elapsed Time-----------------------------------------
startTime <- proc.time()

# set current working directory (as per sony laptop)---------------------------
...
First, lets see the general structure of a typical applicant dataset for a company. Below, I display the  first 4 rows and a few feature columns (I display 4 feature columns, but the dataset contains `r kTotalNumOfFeat` feature columns) followed by a `labels` column for the first applicant dataset (`appDatasetList[[1]]`) corresponding to C~1~. Note that the `labels` column comes at the end (i.e. 21st column), but I display it here just after the first 4 columns for illustration purposes. 

&nbsp;

```{r appDatasetStrDisp, message = FALSE, warning = FALSE}
...
```{r colNamesAppDataset, message = FALSE, warning = FALSE}
...
```{r criteriaNumberRecVecDisp, message = FALSE, warning = FALSE}
...
```{r criteriaNumberRecVecHist, message = FALSE, warning = FALSE}
...
```{r outputModelsList[[1]], message = FALSE, warning = FALSE}
modelsList[[1]]
```
...
```{r afvUser1Expln, message = FALSE, warning = FALSE}
...
```{r myPersLabColDisp, message = FALSE, warning = FALSE}
...
```{r Part-2, message = FALSE, warning = FALSE}
...
```{r compDatasetStrDisp, message = FALSE, warning = FALSE}
colNotToDisplay <- c(5:20)
knitr::kable(
...
```{r colNamesCompDat, message = FALSE, warning = FALSE}
...
```{r persJobClassMod, message = FALSE, warning = FALSE}
...
```{r codeChunkLookLike, echo = FALSE, message = FALSE, warning = FALSE}
...
* Click [here](https://www.dropbox.com/s/619ty2r0cd714a7/PersonalizedJobClassifier.Rmd?dl=0) to download the **.Rmd** file which you can yourself run in R studio to see the code work for yourself

## Useful Links
* Click [here](https://aarsh.shinyapps.io/personalized-job-classifier/) to view the **interactive App** that gives
a user interface to the Personalized Job Classifier model.
...
```{r elapsedTimeInMin, message = FALSE, warning = FALSE}
